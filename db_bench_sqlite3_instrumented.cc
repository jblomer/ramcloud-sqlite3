...
#include <stdlib.h>
//#include <sqlite3.h>
#include "../../../ramcloud-sqlite3/sqlite3.h"
#include "../../../ramcloud-sqlite3/vfs-ramcloud.h"
...
class Benchmark {
 ...
 public:
  ...
  void Open() {
    ...
    //std::string NOJRNL_stmt = "PRAGMA journal_mode=OFF; PRAGMA count_changes=OFF; PRAGMA synchronous=OFF; PRAGMA temp_store=MEMORY";
    //status = sqlite3_exec(db_, NOJRNL_stmt.c_str(), NULL, NULL, &err_msg);
    //printf("changed journal mode %d\n", status);
    //ExecErrorCheck(status, err_msg);
    printf("WE ARE AT THE VERY END OF OPEN\n");
  }
  ...
  void ReadSequential() {
    ...
    printf("----- PREPARE STATEMENTS SEQREAD --------\n");
    status = sqlite3_prepare_v2(db_, read_str.c_str(), -1, &pStmt, NULL);
    ErrorCheck(status);
    printf("----- RUN STATEMENTS SEQREAD --------\n");
    for (int i = 0; i < reads_ && SQLITE_ROW == sqlite3_step(pStmt); i++) {
      printf("----- ITERATION %d SEQREAD --------\n", i);
      bytes_ += sqlite3_column_bytes(pStmt, 1) + sqlite3_column_bytes(pStmt, 2);
      FinishedSingleOp();
    }
    printf("----- FINALIZE STATEMENTS SEQREAD --------\n");
...
int main(int argc, char** argv) {
  ...
  SQLITE_RCVFS_CONNECTION *conn =
    sqlite3_rcvfs_connect("infrc:host=192.168.1.119,port=11100", "main", "leveldb");
  assert(conn != NULL);
  int retval;
